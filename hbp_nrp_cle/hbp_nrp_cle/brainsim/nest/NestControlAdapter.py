...
class NestControlAdapter(IBrainControlAdapter):
    ...
    def __init__(self, sim):
        """
        Initializes the NEST control adapter
    ...
    def load_populations(self, **populations):
        ...
        if tf_config.brain_root is None:
            raise Exception(
                "No brain is currently loaded, cannot add Populations.")
    ...
    def _is_population(self, candidate):
        """
        Determines whether the candidate is a population (i.e. a list of int)
    ...
    def _create_population_info(self, population, name):
        ...
        """
        try:
            celltype = str(self._sim.GetStatus(population, 'model')[0])
            parameters = self._sim.GetDefaults(celltype)
        except self._sim.NESTError:
            parameters = {}
        return NestPopulationInfo(population, name, parameters)
