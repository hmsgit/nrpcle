...
def spawn_sdf_model_client(model_name, model_xml, robot_namespace, initial_pose, reference_frame,
    ...
    try:
        ...
        resp = spawn_sdf_model(model_name, model_xml, robot_namespace, initial_pose,
                               reference_frame)
...
def spawn_urdf_model_client(model_name, model_xml, robot_namespace, initial_pose, reference_frame,
    ...
    try:
        ...
        resp = spawn_urdf_model(model_name, model_xml, robot_namespace, initial_pose,
                                reference_frame)
        rospy.loginfo("Spawn status: %s" % resp.status_message)
        return resp.success
    except rospy.ServiceException as e:
...
def set_model_configuration_client(model_name, model_param_name, joint_names, joint_positions,
    ...
    try:
        set_model_configuration = rospy.ServiceProxy(gazebo_namespace + '/set_model_configuration',
