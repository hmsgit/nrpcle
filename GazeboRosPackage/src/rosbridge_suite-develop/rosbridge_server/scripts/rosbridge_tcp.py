...
# TODO: take care of socket timeouts and make sure to close sockets after
# killing programm to release network ports

# TODO: add new parameters to websocket version! those of rosbridge_tcp.py
# might not be needed, but the others should work well when adding them to
# .._websocket.py
...
class RosbridgeTcpSocket(SocketServer.BaseRequestHandler):
    ...
    def setup(self):
        ...
        try:
            self.protocol = RosbridgeProtocol(
                client_id_seed, parameters=parameters)
            self.protocol.outgoing = self.send_message
            client_id_seed = client_id_seed + 1
            clients_connected = clients_connected + 1
            self.protocol.log(
                "info", "connected. " + str(clients_connected) + " client total.")
 ...
 (ignore warning if client is only listening..)")
    ...
    def finish(self):
        ...
        self.protocol.log("info", "disconnected. " + str(
            clients_connected) + " client total.")
...
# TODO: check if ROS parameter server uses None string for 'None-value' or
# Null or something else, then change code accordingly

            # update parameters from parameter server or use default value (
            # second parameter of get_param )
            ...
            retry_startup_delay = get_param(
                '~retry_startup_delay', retry_startup_delay)
            service_request_timeout = get_param(
                '~service_request_timeout', service_request_timeout)
            check_response_delay = get_param(
                '~check_response_delay', check_response_delay)
            wait_for_busy_service_provider = get_param(
                '~wait_for_busy_service_provider', wait_for_busy_service_provider)
            max_service_requests = get_param(
                '~max_service_requests', max_service_requests)
            ...
            if "--max_message_size" in sys.argv:
                ...
                if idx < len(sys.argv):
                    ...
                    print "--max_message_size argument provided without a value.\
 (can be None or <Integer>)"
                    sys.exit(-1)
            ...
            # empty string for host makes server listen on all available
            # interfaces
            server = SocketServer.ThreadingTCPServer(
                (host, port), RosbridgeTcpSocket)
