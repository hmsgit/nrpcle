...
class OpenIdMixin(object):
    ...
    * ``_OPENID_ENDPOINT``: the identity provider's URI.
    ...
    def authenticate_redirect(self, callback_uri=None,
        ...
        self.redirect(
            self._OPENID_ENDPOINT + "?" + urllib_parse.urlencode(args))
        callback()
...
class TwitterMixin(OAuthMixin):
    ...
    def twitter_request(self, path, callback=None, access_token=None,
        ...
        if post_args is not None:
            http.fetch(
                url, method="POST", body=urllib_parse.urlencode(post_args),
...
def _oauth10a_signature(
    ...
    base_string = "&".join(_oauth_escape(e) for e in base_elems)
    key_elems = [
        escape.utf8(urllib_parse.quote(consumer_token["secret"], safe='~'))]
    key_elems.append(
        escape.utf8(urllib_parse.quote(token["secret"], safe='~') if token else ""))
    key = b"&".join(key_elems)
