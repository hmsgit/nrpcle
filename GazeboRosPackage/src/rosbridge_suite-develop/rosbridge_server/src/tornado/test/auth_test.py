...
from tornado.auth import OpenIdMixin, OAuthMixin, OAuth2Mixin, \
    TwitterMixin, GoogleMixin, AuthError
...
class OAuth1ClientLoginHandler(RequestHandler, OAuthMixin):
    ...
    def initialize(self, test, version):
        ...
        self._OAUTH_REQUEST_TOKEN_URL = test.get_url(
            '/oauth1/server/request_token')
        self._OAUTH_AUTHORIZE_URL = test.get_url('/oauth1/server/authorize')
        self._OAUTH_ACCESS_TOKEN_URL = test.get_url(
...
class TwitterServerShowUserHandler(RequestHandler):
    ...
    def get(self, screen_name):
        ...
        assert self.get_argument(
            'oauth_consumer_key') == 'test_twitter_consumer_key'
        assert self.get_argument('oauth_signature_method') == 'HMAC-SHA1'
        assert self.get_argument('oauth_version') == '1.0'
        assert self.get_argument('oauth_token') == 'hjkl'
        self.write(
            dict(screen_name=screen_name, name=screen_name.capitalize()))
...
class AuthTest(AsyncHTTPTestCase):
    ...
    def get_app(self):
        return Application(
            [
                # test endpoints
                ('/openid/client/login',
                 OpenIdClientLoginHandler, dict(test=self)),
                ...
                ('/oauth10a/client/request_params',
                 ...
                 dict(version='1.0a')),
                ('/oauth2/client/login',
                 OAuth2ClientLoginHandler, dict(test=self)),

                ('/twitter/client/login',
                 TwitterClientLoginHandler, dict(test=self)),
                ...
                ('/google/client/openid_login',
                 GoogleOpenIdClientLoginHandler, dict(test=self)),
    ...
    def test_openid_get_user(self):
        response = self.fetch(
            '/openid/client/login?openid.mode=blah&openid.ns.ax=http://openid.net/srv/ax\
/1.0&openid.ax.type.email=http://axschema.org/contact/email&openid.ax.value.email\
=foo@example.com')
        response.rethrow()
    ...
    def test_oauth10_redirect(self):
        ...
        self.assertTrue(
            '_oauth_request_token="enhjdg==|MTIzNA=="' in response.headers[
                'Set-Cookie'],
    ...
    def test_oauth10_get_user(self):
        ...
        self.assertEqual(parsed['email'], 'foo@example.com')
        self.assertEqual(
            parsed['access_token'], dict(key='uiop', secret='5678'))
    ...
    def test_oauth2_redirect(self):
        ...
        self.assertEqual(response.code, 302)
        self.assertTrue(
            '/oauth2/server/authorize?' in response.headers['Location'])
    ...
    def test_google_redirect(self):
        ...
        response = self.fetch(
            '/google/client/openid_login', follow_redirects=False)
        self.assertEqual(response.code, 302)
        self.assertTrue(
            '/openid/server/authenticate?' in response.headers['Location'])
    ...
    def test_google_get_user(self):
        response = self.fetch(
            '/google/client/openid_login?openid.mode=blah&openid.ns.\
ax=http://openid.net/srv/ax/1.0&openid.ax.type.email\
=http://axschema.org/contact/email&openid.ax.value\
.email=foo@example.com', follow_redirects=False)
        response.rethrow()
        parsed = json_decode(response.body)
        self.assertEqual(parsed["email"], "foo@example.com")
