...
class MultipartTestHandler(RequestHandler):
    ...
    def post(self):
        self.finish({"header": self.request.headers["X-Header-Encoding-Test"],
                     ...
                     "filebody":
                     _unicode(self.request.files["files"][0]["body"]),
                     })
...
class TypeCheckHandler(RequestHandler):
    ...
    def prepare(self):
        ...
        self.check_type(
            'header_key', list(self.request.headers.keys())[0], str)
        self.check_type('header_value', list(
            self.request.headers.values())[0], str)

        self.check_type(
            'cookie_key', list(self.request.cookies.keys())[0], str)
        self.check_type('cookie_value', list(
            self.request.cookies.values())[0].value, str)
        # secure cookies

        self.check_type('arg_key', list(self.request.arguments.keys())[0], str)
        self.check_type(
            'arg_value', list(self.request.arguments.values())[0][0], bytes_type)
...
class UnixSocketTest(AsyncTestCase):
    ...
    def setUp(self):
        ...
        self.stream = IOStream(
            socket.socket(socket.AF_UNIX), io_loop=self.io_loop)
...
class KeepAliveTest(AsyncHTTPTestCase):
    ...
    def read_headers(self):
        ...
        self.assertTrue(first_line.startswith(
            self.http_version + b' 200'), first_line)
        self.stream.read_until(b'\r\n\r\n', self.stop)
...
class BodyLimitsTest(AsyncHTTPTestCase):
    ...
    def test_large_body_streaming(self):
        with ExpectLog(gen_log, '.*Content-Length too long'):
            response = self.fetch(
                '/streaming', method='PUT', body=b'a' * 10240)
        self.assertEqual(response.code, 599)
    ...
    def test_body_size_override_reset(self):
        ...
        try:
            ...
            # Without the ?expected_size parameter, we get the old default
            # value
