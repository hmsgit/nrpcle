...
from tornado.testing import AsyncHTTPTestCase, AsyncHTTPSTestCase,\
    AsyncTestCase, bind_unused_port, ExpectLog
...
class SeeOtherPostHandler(RequestHandler):
    ...
    def post(self):
        ...
        assert redirect_code in (
            302, 303), "unexpected body %r" % self.request.body
...
class SimpleHTTPClientTestMixin(object):
    ...
    def test_see_other_redirect(self):
        for code in (302, 303):
            response = self.fetch(
                "/see_other_post", method="POST", body="%d" % code)
    ...
    def test_request_timeout(self):
        ...
        self.assertTrue(
            0.099 < response.request_time < 0.15, response.request_time)
    ...
    def test_host_header(self):
        ...
        url = self.get_url("/host_echo").replace(
            "http://", "http://me:secret@")
        self.http_client.fetch(url, self.stop)
        response = self.wait()
    ...
    def test_connection_refused(self):
        ...
        if sys.platform != 'cygwin':
            ...
            if not contains_errno and hasattr(errno, "WSAECONNREFUSED"):
                contains_errno = str(
                    errno.WSAECONNREFUSED) in str(response.error)
