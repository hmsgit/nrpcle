...
from tornado.escape import json_decode, utf8, to_unicode,\
    recursive_unicode, native_str, to_basestring
...
from tornado.web import RequestHandler, authenticated, Application,\
    asynchronous, url, HTTPError, StaticFileHandler,\
    _create_signature_v1, create_signed_value,\
    decode_signed_value, ErrorHandler, UIModule,\
    MissingArgumentError, stream_request_body, Finish
...
class CookieTestRequestHandler(RequestHandler):
    ...
    def __init__(self):
        ...
        self.application = ObjectDict(
            settings=dict(cookie_secret='0123456789'))
...
class SecureCookieV1Test(unittest.TestCase):
    ...
    def test_arbitrary_bytes(self):
        ...
        self.assertEqual(
            handler.get_secure_cookie('foo', min_version=1), b'\xe9')
...
class AuthRedirectRequestHandler(RequestHandler):
    ...
    @authenticated
    def get(self):
        # we'll never actually get here because the test doesn't follow
        # redirects
        self.send_error(500)
...
class AuthRedirectTest(WebTestCase):
    ...
    def test_relative_auth_redirect(self):
        ...
        self.assertEqual(
            response.headers['Location'], '/login?next=%2Frelative')
...
class TypeCheckHandler(RequestHandler):
    ...
    def prepare(self):
        ...
        self.check_type('cookie_value', list(
            self.cookies.values())[0].value, str)
        ...
        if list(self.cookies.keys()) != ['asdf']:
            raise Exception("unexpected values for cookie keys: %r" %
                            self.cookies.keys())
        self.check_type(
            'get_secure_cookie', self.get_secure_cookie('asdf'), bytes_type)
        ...
        self.check_type(
            'reverse_url', self.reverse_url('typecheck', 'foo'), str)
...
{{ set_resources(embedded_css=".entry { margin-bottom: 1em; }",
 embedded_javascript="js_embed()", css_files=["/base.css",
 "/foo.css"], javascript_files="/common.js", html_head="<meta>",
...
<html><head><link href="/base.css" type="text/css" rel="stylesheet"/>
...
<script src="/analytics.js"/>
    ...
    def test_get_argument(self):
        ...
        # In singular form, body arguments take precedence over query
        # arguments.
        body = urllib_parse.urlencode(dict(foo="hello"))
        response = self.fetch(
            "/get_argument?foo=bar", method="POST", body=body)
...
class StaticFileTest(WebTestCase):
    ...
    def get_handlers(self):
        ...
        class OverrideStaticUrlHandler(RequestHandler):
            ...
            def get(self, path):
                ...
                check_override = override_url.find(
                    protocol, 0, protocol_length)
    ...
    def test_static_with_range_partial_past_end(self):
        ...
        self.assertEqual(
            response.headers.get("Content-Range"), "bytes 1-25/26")
...
class Default404Test(WebTestCase):
    ...
    def get_handlers(self):
        # If there are no handlers at all a default redirect handler gets
        # added.
