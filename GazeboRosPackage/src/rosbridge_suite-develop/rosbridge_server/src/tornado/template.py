...
class Template(object):
    ...
    def _generate_python(self, loader, compress_whitespace):
        ...
        try:
            ...
            writer = _CodeWriter(
                buffer, named_blocks, loader, ancestors[0].template,
                compress_whitespace)
...
class _File(_Node):
    ...
    def generate(self, writer):
        ...
        with writer.indent():
            ...
            writer.write_line(
                "return _tt_utf8('').join(_tt_buffer)", self.line)
...
class _IntermediateControlBlock(_Node):
    ...
    def generate(self, writer):
        ...
        writer.write_line("pass", self.line)
        writer.write_line("%s:" %
                          self.statement, self.line, writer.indent_size() - 1)
    ...
    while True:
        ...
        # End tag
            ...
            if operator == "extends":
                ...
                if not suffix:
                    raise ParseError(
                        "extends missing file path on line %d" % line)
                block = _ExtendsBlock(suffix)
            elif operator in ("import", "from"):
                if not suffix:
                    raise ParseError(
                        "import missing statement on line %d" % line)
                block = _Statement(contents, line)
            elif operator == "include":
                suffix = suffix.strip('"').strip("'")
                if not suffix:
                    raise ParseError(
                        "include missing file path on line %d" % line)
            ...
            if operator == "apply":
                if not suffix:
                    raise ParseError(
                        "apply missing method name on line %d" % line)
