...
class UnadvertiseService(Capability):
    ...
    def __init__(self, protocol):
        ...
        protocol.register_operation(
            "unadvertise_service", self.unadvertise_service)
    ...
    def unadvertise_service(self, message):
        ...
        if service_name in self.protocol.external_service_list.keys():
            self.protocol.external_service_list[
                service_name].service_handle.shutdown("Unadvertise request.")
            del self.protocol.external_service_list[service_name]
            self.protocol.log(
                "info", "Unadvertised service %s." % service_name)
        else:
            self.protocol.log(
                "error", "Service %s has no been advertised externally." % service_name)
