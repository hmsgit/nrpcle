...
class Defragment(Capability, threading.Thread):
    ...
    def __init__(self, protocol):
        ...
        if self.protocol.parameters is not None:
            self.fragment_timeout = self.protocol.parameters[
                "fragment_timeout"]
    ...
    def defragment(self, message):
        ...
        if self.received_fragments is not None:
            for id in self.received_fragments.keys():
                time_diff = now - \
                    self.received_fragments[id]["timestamp_last_append"]
                if (time_diff.total_seconds() > self.fragment_timeout and
                    ...
                    if message["id"] != id:
                        ...
                        log_msg += str(
                            self.received_fragments[message.get("id")]["total"])
        ...
        if ((msg_opcode is None) or (msg_id is None) or
            (msg_num is None) or (msg_total is None) or
                (msg_data is None)):
        ...
        # print "received fragments:",
        # len(self.received_fragments[msg_id]["fragment_list"].keys())
        ...
        existing_fragments = len(
            self.received_fragments[msg_id]["fragment_list"])
        ...
        if existing_fragments == announced_total:
            log_msg = "enough/all fragments for messageID " + \
                str(msg_id) + " received"
            ...
            for i in range(0, announced_total):
                if i not in self.received_fragments[msg_id]["fragment_list"]:
                    ...
                    log_msg += " for messageID " + \
                        str(msg_id) + " is missing! "
                    self.protocol.log("error", log_msg)

        self.received_fragments[msg_id][
            "is_reconstructing"] = received_all_fragments
        ...
        if received_all_fragments:
            ...
            for i in range(0, message["total"]):
                reconstructed_msg += self.received_fragments[
                    msg_id]["fragment_list"][i]
            ...
            # str(len(str(json.loads(reconstructed_msg)["msg"]["data"]))) +"]"
