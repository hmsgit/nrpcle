...
class ServiceTester:
    ...
    def start(self):
        ...
        services.ServiceCaller(
            self.name, gen, self.success, self.error).start()
...
class TestServices(unittest.TestCase):
        ...
        for srv_type in ["TestRequestOnly", "TestRequestAndResponse"]:
            ...
            for args in [[3], {"data": 3}]:
                ...
                services.args_to_service_request_instance(
                    "", cls._request_class(), args)
            self.assertRaises(
                FieldTypeMismatchException, services.args_to_service_request_instance, "",
    ...
    def test_service_caller(self):
        ...
        # Now, call using the services
        services.ServiceCaller(
            "/rosout/get_loggers", None, success, error).start()
    ...
    def test_service_tester_alltypes(self):
        ts = []
        for srv in [
            "TestResponseOnly", "TestEmpty", "TestRequestAndResponse", "TestRequestOnly",
